...
_isInCombat = player getVariable["startcombattimer",0]; if (_isInCombat == 1) then { 
    cutText [format["You are in Combat and cannot upgrade your Merlin."], "PLAIN DOWN"];
	sleep 2;
	cutText [format["Make sure you upgrade your Merlin somewhere safe and clear of objects."], "PLAIN DOWN"];
} else {
	...
	if (_has2 && _has1) then {
			...
			cutText [format["You've upgraded to a Huey!"], "PLAIN DOWN"];
