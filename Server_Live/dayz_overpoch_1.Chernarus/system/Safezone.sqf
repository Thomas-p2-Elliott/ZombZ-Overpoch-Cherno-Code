...
Features:
	Anti-Spam: Godmode not re-enabled for 30 seconds if player has re-entered trader-zone twice within last 30 seconds
	Protection Disable Delay: After leaving the trader zone protection remains enabled with speed limit disabled for 30 seconds
	God / Vehicle God
	No Firing Weapons/Vehicle Weapons
	Dual Speed Limit (40kph with no players near, 25kph with players near)
	No Loot / Zed Spawns
	No Clothing Changing
	Anti-Backpack Theft
...
while {true} do {
	...
	/* Player God Mode */
	if (p2sz_asC > 1) then{
		systemChat ("[ZombZ] Antispam - Please wait before re-entering!");
	} else {
		p2sz_asC = p2sz_asC + 1; //player has entered safezone
		systemChat ("[ZombZ] Entering Trader Area - God Mode Enabled");
	...
	_vehWepDisableThread = [] spawn {
		...
		while {szDisableVehFire} do
		{
			...
			{
				systemChat ("[ZombZ] No Firing Vehicle Guns Enabled");
		...
		while {true} do
		{
			...
			if( !isNull (FindDisplay 106) && !_skip ) then
			{
				if (isNull p2sz_asC_lLat) then
				{
					...
					systemChat ("[ZombZ] Anti Backpack Stealing - Redirecting you to your own gear!");
					waitUntil { isNull (FindDisplay 106) };
				} else {
					systemChat (format["[ZombZ] You cannot open your gear at this time as you have looked at a player in the last 5 seconds."]);
					(findDisplay 106) closeDisplay 1;
					waitUntil { isNull (FindDisplay 106) };
				};
			};
			if (_skip && _if) then {
				systemChat ("[ZombZ] This player is tagged friendly, you have access to this players bag");
				uiSleep 30;
			};
		};
	};
	...
	zombzsafezone = 2; //Change Status from out of safe-zone (0) to timer in progress

	systemChat ("[ZombZ] Exiting Trader Area - Protection will remain enabled for 30 seconds!");

	//Start Timer Thread
	_timerThread = [] spawn { uiSleep 30; zombzsafezone = 0; };

	waitUntil {zombzsafezone < 2};
	/*---------------------------------------------------------------------------
