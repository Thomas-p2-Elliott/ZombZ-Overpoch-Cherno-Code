...
if (!dayz_zedsAttackVehicles && !(_vehicle in _targets) && !(_vehicle isKindOf "Bicycle") && !(_vehicle isKindOf "ATV_Base_EP1") && !(_vehicle isKindOf "Motorcycle")) exitWith { 
	//diag_log ("not attacking vehicle" + str(_vehicle));  
};

if ((speed _vehicle > 7)) exitWith { 
};
