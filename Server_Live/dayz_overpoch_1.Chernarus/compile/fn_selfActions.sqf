...
if (!isNull cursorTarget && !_inVehicle && !_isPZombie && (player distance cursorTarget < _allowedDistance) && _canDo) then {	//Has some kind of target
	...
	if(_isAlive) then {
		...
		if(_isDestructable || _isWreck || _isRemovable || _isWreckBuilding) then {
			...
			//Allow player to check a buildings health
		...
		if (s_player_lockUnlock_crtl < 0) then {
			...
			if(locked _cursorTarget) then {
				if(_hasKey || _oldOwner) then {
					...
					if(_hasHotwireKit && !isInTraderCity) then {
	...
	if(_isTent && (player distance _cursorTarget < 3)) then {
		if (_ownerID == _playerUID) then {
			...
			if(("ItemJerrycan" in _magazinesPlayer) && ("ItemMatchbox_DZE" in weapons player)) then {
				if (s_player_packtent < 0) then {
					s_player_packtent = player addAction [localize "STR_EPOCH_ACTIONS_DESTROYTENT", "actions\remove.sqf",_cursorTarget, 1, true, true, "", ""];
				};
			};
		};
	} else {
	...
	//Fuel Pump
	if(_typeOfCursorTarget in dayz_fuelpumparray || _isFuel) then {	
	...
	if((_isDestructable || _cursorTarget isKindOf "Land_DZE_WoodDoorLocked_Base" || _cursorTarget isKindOf "CinderWallDoorLocked_DZ_Base") && (DZE_Lock_Door == _characterID)) then {
		...
		if (s_player_downgrade_build < 0) then {
			...
			s_player_downgrade_build = player addAction [format[localize "STR_EPOCH_ACTIONS_REMLOCK",_text], "actions\player_buildingDowngrade.sqf",_cursorTarget, -2, false, true, "",""];
		};
	} else {
		player removeAction s_player_downgrade_build;
		s_player_downgrade_build = -1;
	...
	if (_isMan && !_isPZombie && _traderType in serverTraders) then {
		...
		if (s_player_parts_crtl < 0) then {
			...
			if(_humanity_logic) then {
				...
				s_player_parts set [count s_player_parts,_buy];
				_buyV = player addAction ["<t color='#0059FF'>Advanced Trading</t>", "system\advancedTrading\openMenu.sqf",(_traderMenu select 0), 999, true, false, "",""];
