...
_agent addEventHandler ["local", { diag_log "Locality Event"; if(_this select 1) then {[(position (_this select 0)),(_this select 0),true] execFSM ("" + (P2HC_Path) + "zeds\zombie_agentHC.fsm") };}];
...
//Start behavior
_id = [_position,_agent] execFSM ("" + (P2HC_Path) + "zeds\zombie_agentHC.fsm");
