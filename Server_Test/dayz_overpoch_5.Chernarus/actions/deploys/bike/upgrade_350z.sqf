...
_isInCombat = player getVariable["startcombattimer",0]; if (_isInCombat == 1) then { 
	...
	sleep 2;
	cutText [format["Make sure you upgrade your motorcycle somewhere safe and clear of objects."], "PLAIN DOWN"];
} else {
	...
	if (_has2 && _has1) then {
			...
			cutText [format["You've upgraded to a 350z!"], "PLAIN DOWN"];
			...
			cutText [format["Warning: Spawned 350z do Not Save after server restart!"], "PLAIN DOWN"];
			
		} else {
			cutText [format["You need an Wheel and Scrap Metal to build this."], "PLAIN DOWN"];
			sleep 5;
			cutText [format["Make sure you build your 350z somewhere safe and clear of objects."], "PLAIN DOWN"];
		};
