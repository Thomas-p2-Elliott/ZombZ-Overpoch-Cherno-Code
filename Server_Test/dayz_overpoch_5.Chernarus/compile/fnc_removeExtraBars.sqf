...
_d = P2DZE_goldItemHandlingDebug;						//enable debug output?
...
if (_timeSinceLast < 0.4 || P2DZE_goldRunning) exitWith {
	...
	if (_keep1Bar) then {
		...
		if (_goldBarCount > 1) then {
			for "_i" from 1 to (_goldBarCount - 1) do {
				...
				if (!P2DZE_goldRunning) then { [] spawn player_pickupGold; };
			};
			//new
			call player2_preventInvOverfill;
		} else {
			if (_goldBarCount < 1) then {
