...
_uid call {
	...
	if (typeName _this != typeName 'String') exitWith {
		//diag_log('UID was not string');
	};
	if ((count (toArray(_this))) < 6) exitWith {
		//diag_log('UID was less than 6 chars long');
	};
	_uid = format["%1",_this];
	if (isNil '_uid') exitWith {
		//diag_log('UID was nil');
	};
