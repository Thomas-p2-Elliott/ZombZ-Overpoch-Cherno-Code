private ["_obj","_dis","_sfx","_object","_dir","_pos","_pwep","_mags","_magtype","_mammo","_has1","_has2","_isInCombat"];
...
_isInCombat = player getVariable["startcombattimer",0]; if (_isInCombat == 1) then { 
	...
	sleep 2;
	cutText [format["Make sure you upgrade your BTR (m240) somewhere safe and clear of objects."], "PLAIN DOWN"];
} else {
	...
	if (_has2 && _has1) then {
			...
			cutText [format["You've upgraded to a BTR (m240)!"], "PLAIN DOWN"];
			...
			cutText [format["Warning: Spawned BTRs (m240) do Not Save after server restart!"], "PLAIN DOWN"];
