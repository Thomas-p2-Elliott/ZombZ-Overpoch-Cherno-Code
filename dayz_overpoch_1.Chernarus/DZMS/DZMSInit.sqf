...
if (!isnil("P2DZMS_startWait")) then {
	uiSleep P2DZMS_startWait;
} else {
	uiSleep 120;
};

// Error Check
//if (!isServer || hasInterface) exitWith { diag_log text format ["[DZMS]: <ERROR> DZMS is Installed Incorrectly! DZMS is not Running!"]; };
...
call compile preprocessFileLineNumbers "DZMS\DZMSFunctions.sqf";
