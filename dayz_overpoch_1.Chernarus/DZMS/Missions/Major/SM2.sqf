...
P2DZ_mH = ["1", "A plane with Bandit Supplies is Flying In!", _gps, "AN2_DZ"];
...
while {_loop} do {
	if (!Alive _plane OR !Alive _pilot) then {
		...
		P2DZ_mH = ["1", "The AN2 was shot down by UN Peacekeepers! Go get the supplies!", _gps, "AN2_DZ"];
		publicVariable "P2DZ_mH";
	...
	if ((Alive _plane) AND (Alive _pilot) AND ((_plane distance _wp_pos) <= 1200) AND (!(_half))) then {
		P2DZ_mH = ["1", "The AN2 is only 1200m out from the drop point! ", _gps, "AN2_DZ"];
		publicVariable "P2DZ_mH";
		//Keep on truckin'
		_plane forceSpeed 175;
	...
	if ((Alive _plane) AND (Alive _pilot) AND ((_plane distance _wp_pos) <= 200)) then {
		...
		//Let everyone know the mission is over
		P2DZ_mH = ["1", "The AN2 has reached the location and dropped the cargo!", _gps, "AN2_DZ"];
		publicVariable "P2DZ_mH";
...
//Let everyone know the mission is over
P2DZ_mH = ["1", "The AN2 Cargo has been Secured by Survivors!", _gps, "AN2_DZ"];
publicVariable "P2DZ_mH";
