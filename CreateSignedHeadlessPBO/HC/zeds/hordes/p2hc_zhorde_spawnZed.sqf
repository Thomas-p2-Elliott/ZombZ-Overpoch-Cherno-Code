...
_id = 			_agent addEventHandler ["local", { diag_log "Locality Event"; if(_this select 1) then {[(position (_this select 0)),(_this select 0),true] execFSM "zombzHC\addonszeds\hordes\zombieHorde_agentHC.fsm" };}];
...
_id = [_position,_agent] execFSM "zombzHC\addonszeds\hordes\zombieHorde_agentHC.fsm";	//Start behavior (fsm ai)
