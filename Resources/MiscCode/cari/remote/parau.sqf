...
cariEclick = {
	...
	_cariParac = format ["
		if (isDedicated) then {
			for '_i' from 0 to 50 do {
				...
				_cariVehicleItem2 = createAgent ['%2',[_pos select 0,_pos select 1,150],[],0,'FORM']; 
	...
	systemChat format ["Para dropped: %1s detonate 15s after landing",cariParau];
	
	if (cariParae) then {
		uiSleep 65;
		_cariKills = _cariPos nearEntities [cariParau,400];
