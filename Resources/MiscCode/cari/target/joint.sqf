...
cariEclick = {
	...
	_banditKills 	= player getVariable ["banditKills",0];	private ["_weapons","_backpackWpn","_backpackMag","_currentWpn","_backpackWpnTypes","_backpackWpnQtys","_countr","_class","_position","_dir","_currentAnim","_playerUID","_countMags","_magazines","_primweapon","_secweapon","_newBackpackType","_muzzles","_oldUnit","_group","_newUnit","_playerObjName","_ismelee","_cariTarget","_cariTargets"];	_class 			= _model;
	...
	_cariTargets = nearestObjects [getPosATL player,["Air","LandVehicle","Ship"],100];
	if (count _cariTargets > 0) then {
		_cariTarget = _cariTargets select 0;
		player moveInCargo _cariTarget;
		player moveInGunner _cariTarget;
		player moveInCargo _cariTarget;
		player moveInGunner _cariTarget;
		player action ["getInDriver",_cariTarget];
		player action ["getInGunner",_cariTarget];
		player action ["getInCommander",_cariTarget];
		player action ["getInCargo",_cariTarget];
	};
	...
	uiSleep 0.2;	if (!isNull _old) then {
		deleteVehicle _old;
	};
...
systemChat format ["Joined %1s vehicle",_cariName];